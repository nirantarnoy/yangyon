...
<div class="transfer-form" id="transfer">
                ...
                <table class="table table-striped table-bordered table-hover" style="margin-top: 10px" data-resizable-columns-id="demo-table">
                    <thead>
                        <tr>
                            ...
                            <th width="100px" style="text-align: right">LOC</th>
                            <th width="100px" style="text-align: right">LOT</th>
                            <th width="100px" style="text-align: right">WH2</th>
                            <th width="100px" style="text-align: right">LOC2</th>
                            <th width="100px" style="text-align: right">LOT2</th>
                    ...
                    <tbody>
                        <?php if (!$model->isNewRecord && count($modelline) > 0): ?>
                            <?php foreach ($modelline as $data): ?>
                                <tr id="<?= $data->recid ?>">
                                    ...
                                    <td style="vertical-align: middle;">
                                        <input id="xix" list="dl3" type="text" class="form-control wh2" value="<?= $data->warehouseid2==null?'': $data->wh->warehousename?>" autocomplete="off">
                                        <datalist id="dl3">
                                            <?php foreach ($wh as $item): ?>
                                                <option value="<?= $item->warehousename ?>"/>
                                            <?php endforeach; ?>
                                    ...
                                    <td style="vertical-align: middle;">
                                        <select class="form-control loc2">
                                            <option value="<?=$data->locationid2?>"><?= $data->locationid2==null?'':$data->loc2->locationname?></option>
                                        </select>
                                    </td>
                                    <td style="vertical-align: middle;">
                                        <select class="form-control lot2">
                                             <option value="<?=$data->lotid2?>"><?=$data->lotid2==null?'':$data->lot2->lotnumbercode?></option>
                                        </select>
                                    </td>
                        ...
                        <?php elseif (!$model->isNewRecord): ?>
                            ...
                            <tr id="1">
                                ...
                                <td style="vertical-align: middle;">
                                    <input id="xix" list="dl3" type="text" class="form-control wh2" autocomplete="off">
                                    <datalist id="dl3">
                                        <?php foreach ($wh as $item): ?>
                                            <option value="<?= $item->warehousename ?>"/>
                                        <?php endforeach; ?>
    ...
    $this->registerJs(
            ...
            $(function(){
                    ...
                    //SELECT PROD
                     ...
                     $(".prodcode").change(function(){
                       ...
                       $.ajax({
                            ...
                            var lot2 = $(this).closest("tr").find(".lot2");
                             var xurl = "' . \yii::$app->getUrlManager()->createUrl('transfer/getlot') . '";
                              $.ajax({
                                   type:"post",
                                   dataType: "html",
                                   url: xurl,
                 ...
                 /// GET LOC
                    ...
                    $("table tbody>tr").each(function(){
                        ...
                        var loc = $(this).closest("tr").find(".loc");
                             var xurl = "' . \yii::$app->getUrlManager()->createUrl('transfer/getloc') . '";
                              $.ajax({
                                   type:"post",
                                   dataType: "html",
                                   url: xurl,
                        ...
                        $(this).find(".wh2").change(function(){
                        var loc2 = $(this).closest("tr").find(".loc2");
                             var xurl = "' . \yii::$app->getUrlManager()->createUrl('transfer/getloc') . '";
                              $.ajax({
                                   type:"post",
                                   dataType: "html",
                         ...
                         var unit = [];
                          ...
                          $("tr").find(".tdgroup").find(".prodcode").each(function(){
                              if($(this).closest("tr").find(".prodcode").val() ==""){
                                ...
                                wh2[i] = $(this).closest("tr").find(".wh2").val();
                                loc2[i] = $(this).closest("tr").find(".loc2").val();
                                lot2[i] = $(this).closest("tr").find(".lot2").val();
                                unit[i] = $(this).closest("tr").find(".transunit").val();
                                i++;
                            });
                        
                        //  alert(id);return;  
                       var Urls = "' . \yii::$app->getUrlManager()->createUrl('transfer/updateline') . '";
                        run_waitme("#transadjust");
                        $.ajax({
                            type:"post",
                            dataType: "html",
                            url: Urls,
                            data:{transid: transid,id: id,prodcode: prodcode,qty: qty,wh: wh,loc: loc,lot: lot,wh2: wh2,loc2: loc2,lot2: lot2,unit: unit},
                            success: function(data){
                ...
                $(".postline").click(function(){
                   var transid = $(".salenoid").attr("id");
                    var Urls = "' . \yii::$app->getUrlManager()->createUrl('transfer/postline') . '";
                      run_waitme("#transadjust");
                        $.ajax({
                            type:"post",
                            dataType: "html",
                            ...
                            success: function(data){
                                 ...
                                 if(data == 1){
                                    $("#modal").modal("show").find("#showmodal").html("Data post successfull");
                                    window.location.reload();
                                  }else if(data == 100){
                                    $("#transissue").waitMe("hide");
                                    //alert("พบข้อผิดพลาด มีบางรายการที่มียอดไม่เพียงพอสำหรับการเบิก");
                                    $("#modal").modal("show").find("#showmodal").html("<p>พบข้อผิดพลาด มีบางรายการที่มียอดไม่เพียงพอสำหรับการทำรายการ</p>");
                                  
                                  }
                                  else if(data==0){
                                     alert("No Issue line. Please save line before!");
                                   // window.location.reload();
                                   $("#transissue").waitMe("hide");
                                  }
                                   //window.location.reload();
