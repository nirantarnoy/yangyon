...
<div class="transadjust-form" id="transissue">

    <?php $form = ActiveForm::begin(); ?>
    <div class="salenoid" id="<?= $model->isNewRecord ? '' : $session['recid']; ?>"></div>
    <div class="salestatus" id="<?= $model->status; ?>"></div>
    <div class="lineonhand" id=""></div>
...
//                            'model' => $model,
//                            'attribute' => 'transdate',
//                          //  'value' => date("Y-m-d H:i:s"),// date('dd-mm-yyyy', strtotime('+2 days')),
...
//                        ]);
    ...
    <?php
        ...
        echo "<div id='showmodal' style='color: green'></div>";
        echo "<div id='showmodal2' style='color: red'></div>";
?>
<?php Modal::end();?>                               
<?php
$this->registerJs(
                 ...
                 /// GET LOC
                    ...
                    $("table tbody>tr").each(function(){
                        ...
                        var loc = $(this).closest("tr").find(".loc");
                             var xurl = "' . \yii::$app->getUrlManager()->createUrl('transissue/getloc') . '";
                              $.ajax({
                                   type:"post",
                                   dataType: "html",
                                   url: xurl,
                   ...
                   // alert(rowcount);
                    if(rowcount <= 0){
                      ...
                      return;
                        ...
                        //  alert(id);return;  
                       var Urls = "' . \yii::$app->getUrlManager()->createUrl('transissue/updateline') . '";
                        run_waitme("#transissue");
                        $.ajax({
                            type:"post",
                            dataType: "html",
                            url: Urls,
                    ...
                    $(".postline").click(function(){
                        var transid = $(".salenoid").attr("id");
                        var Urls = "' . \yii::$app->getUrlManager()->createUrl('transissue/postline') . '";
                       run_waitme("#transissue");
                        $.ajax({
                            type:"post",
                            dataType: "html",
                            ...
                            success: function(data){
                                 if(data == 1){
                                    ...
                                    $("#modal").modal("show").find("#showmodal2").html("<p>พบข้อผิดพลาด มีบางรายการที่มียอดไม่เพียงพอสำหรับการเบิก</p>");
...
//                        var trqty = $(".lineonhand");
//                        
//                        if(loc == "" || wh ==""){return;}else{
//                            var Urls = "' . \yii::$app->getUrlManager()->createUrl('transissue/checkonhand') . '";
//                       // run_waitme("#pkdetail");
//                        $.ajax({
//                            type:"post",
//                            dataType: "json",
...
//                    });
//                }
//                  $(".transqty").change(function(){
//                        var onhand = $(".lineonhand").attr("id");
//                        var qty = $(this).closest("tr").find(".transqty").val();
//
//                        if(parseInt(qty) > parseInt(onhand)){
//                            alert("จำนวนไม่พอสำหรับการขาย");return;
//                        }
//                        
